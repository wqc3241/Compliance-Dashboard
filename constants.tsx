...
export const MOCK_TRANSACTIONS: Transaction[] = Array.from({ length: 20 }).map((_, i) => {
  const status = i % 6 === 0 ? TransactionStatus.PENDING : (i % 15 === 0 ? TransactionStatus.BLOCKED : TransactionStatus.APPROVED);
  // Deterministically link high-risk transactions to cases for the showcase
  let caseId = undefined;
  if (status === TransactionStatus.PENDING || status === TransactionStatus.BLOCKED) {
    caseId = MOCK_CASES[i % MOCK_CASES.length].id;
  }
